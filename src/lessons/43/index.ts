...
const fillScene = () => {
  ...
  ambientLight.name = 'ambient_light';
	...
	directionalLight.position.set(700, 3000, 1200);
  directionalLight.name = 'light';

	scene.add(ambientLight);
	scene.add(directionalLight);

  const teapotSize = 600;
	const materialColor = new Color();
	materialColor.setRGB(1.0, 0.8, 0.6);

	const phongMaterial = createShaderMaterial(directionalLight, ambientLight);
...
const render = (params: {
  ...
  teapotMaterial.uniforms.shininess.value = state.shininess;
	teapotMaterial.uniforms.uDropoff.value = state.dropoff;
...
const setupGui = (onChange: () => void) => {
  ...
  materialControlFolder.add(state, 'dropoff', 0.0, 2.0, 0.025).name('dropoff');
  materialControlFolder.add(state, 'ka', 0.0, 1.0, 0.025).name('Ka');
  materialControlFolder.add(state, 'kd', 0.0, 1.0, 0.025).name('Kd');
  materialControlFolder.add(state, 'ks', 0.0, 1.0, 0.025).name('Ks');
