...
const createLights = (params: {
  ...
  const angleInRad = Math.PI / 180 * state.angle;
  const directionalLightX = Math.cos(angleInRad);
  const directionalLightZ = Math.sin(angleInRad);
	directionalLight.position.set(directionalLightX, 1, directionalLightZ);
...
const setupGui = (onChange: () => void) => {
  const gui = new GUI();
  const rotationAngleController = gui.add(state, 'angle', 0.0, 360.0).name('rotation angle');
  rotationAngleController.onChange(onChange)
};
