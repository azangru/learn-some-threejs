...
import grassTextureImageUrl from './grass512x512.jpg';
import tailTextureImageUrl from './feather.png';
...
const main = async () => {
  ...
  renderer.shadowMap.enabled = true;
	...
	// CAMERA
	const camera = new PerspectiveCamera(35, canvasRatio, 1, 8000);
  camera.position.set(1230, 920, 520);
...
const getGrassTexture = async () => {
  const texture = await loadTexture(grassTextureImageUrl);
...
const getTailTexture = async () => {
  return await loadTexture(grassTextureImageUrl);
};
...
const fillScene = async () => {
  ...
  const solidGround = new Mesh(
		...
		new MeshLambertMaterial({ color: 0xFFFFFF, map: await getGrassTexture() }));
  ...
  const drinkingBird = await createBird();
...
const createTail = async () => {
  const tailTexture = await loadTexture(tailTextureImageUrl);
  const tail = new Mesh(
...
const createLights = (params: {
  ...
  directionalLight1.position.set( 200, 500, 500 );
  const directionalLight2 = new DirectionalLight(0xFFFFFF, 0.9);
